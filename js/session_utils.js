...
const reportEndpoint =
  ...
  "https://brain-std.azurewebsites.net/service/api/pbi/GetEmbedInfo?parmPbiAppId=1&workspaceId=4dcc64c7-26c2-45d1-b16b-b7a813fe8d63&reportId=8c2e5a47-2670-4b43-b587-fb1de380440e&userName=portaladm@in4ucloud.com&userPassword=Axi4@cns!";
...
function populateEmbedConfigIntoCurrentSession(updateCurrentToken) {
  ...
  return $.getJSON(reportEndpoint, function (embedConfig) {
    handleNewEmbedConfig(JSON.parse(embedConfig.result), updateCurrentToken);
  });
}
...
function handleNewEmbedConfig(embedConfig, updateCurrentToken) {
  ...
  setConfig(
    ...
    "8c2e5a47-2670-4b43-b587-fb1de380440e"
  );
  if (updateCurrentToken) {
    ...
    if (bookmarkContainer) {
      ...
      bookmarkReport.setAccessToken(embedConfig.accessToken);
    }
  }
...
function setTokenExpirationListener() {
  ...
  if (timeout <= 0) {
    ...
    console.log(
      `Report Embed Token will be updated in ${timeout} milliseconds.`
    );
    setTimeout(function () {
      populateEmbedConfigIntoCurrentSession(true /* updateCurrentToken */);
    }, timeout);
